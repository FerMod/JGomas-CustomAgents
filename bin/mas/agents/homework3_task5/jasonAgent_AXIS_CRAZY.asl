...
// Type of troop. In order to differentiate it from other agents, it is a FIELDOPS.
type("CLASS_FIELDOPS").
...
//Sends messages to the other axis telling its position.
...
//Moves to a random position that is not further than 48 meters from its current position.
+!update_targets
	<-  ?manager(M);
		Rad = 48;
		...
		?newPos(NewPosX, NewPosZ);
		!add_task(task("TASK_GOTO_POSITION", M, pos(NewPosX, 0, NewPosZ), ""));
		.
