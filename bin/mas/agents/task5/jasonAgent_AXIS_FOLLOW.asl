...
//Tries to follow the crazy agent.
+!update_targets 
	<-	?manager(M);
		!go_to_crazy(M).

//If it knows the current position of the crazy agent and it's further than 7 meters from it, it follows the crazy agent.
+!go_to_crazy(M): crazy_pos(X,Y,Z) <-	-crazy_pos(X,Y,Z);
										?my_position(MX,MY,MZ);
										!distance(pos(X,Y,Z),pos(MX,MY,MZ));
										?distance(Dist);
										if(Dist >= 7){
											!add_task(task("TASK_GOTO_POSITION", M, pos(X, Y, Z), ""));
										}.

//Otherwise, performs the default task.										
+!go_to_crazy(M).
...
//If it receives a message with the position of the crazy agent, updates the last known position and returns to the "standing" state.
+goto(X,Y,Z)[source(A)] 
	<-  -+crazy_pos(X,Y,Z);
		-+state(standing);
		-goto(X,Y,Z)[source(A)].
