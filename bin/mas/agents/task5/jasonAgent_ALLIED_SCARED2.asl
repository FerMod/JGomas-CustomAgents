...
//Indicates that the agent is waiting in the base.
in_base.
...
//Checks if there is any enemy or flag in the FOV that is closer than 20 meters. If it is, adds a "scared" belief and returns to the "standing" state.
+!perform_look_action: not in_base <-   ?fovObjects(FOVObjects);
										.length(FOVObjects, L);
										+iterador(0);
										while(not scared & iterador(C) & C < L){
											.nth(C, FOVObjects, Objeto);
											.nth(1, Objeto, Equipo);
											.nth(6, Objeto, Pos);
											?my_position(MyPosX, MyPosY, MyPosZ);
											!distance(pos(MyPosX, MyPosY, MyPosZ), Pos);
											?distance(Dist);
											if((Equipo == 200 | Equipo == 0) & Dist < 20){
												+scared;
												-+state(standing);
											}
											-+iterador(C+1);
...
+!setup_priorities
    <-  +task_priority("TASK_NONE",0);
        ...
        +task_priority("TASK_GET_OBJECTIVE",2400);
        +task_priority("TASK_ATTACK", 1000);
        +task_priority("TASK_RUN_AWAY", 2500);
...
//If the agent is waiting in the base, deletes all the tasks and sends a warning to the other scared agent.
+!update_targets: in_base <- -+tasks([]);
							 if(not aviso){
							 	+aviso;
							 }.

//If the agent is scared, it returns to the base.
+!update_targets: scared <-	-scared;
							+in_base;
							?manager(M);
							?initial_indicator(X,Y,Z);
							!add_task(task("TASK_RUN_AWAY", M, pos(X,Y,Z), "")).

//Otherwise, goes to the objective.
+!update_targets <- -aviso;
					?manager(M);
					?objective(X,Y,Z);
					!add_task(task("TASK_GET_OBJECTIVE", M, pos(X,Y,Z), "")).
...
//Sends a warning to the other scared agent.
+aviso <- .my_team("ALLIED", E1);
		  .concat("move", Content1);
		  .send_msg_with_conversation_id(E1, tell, Content1, "INT").

//If it receives a warning from the other scared agent, removes the "in_base" belief and returns to the "standing" state.
+move[source(A)] <- -in_base;
					-+state(standing);
					-move[source(A)].
