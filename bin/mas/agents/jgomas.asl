...
+shooting_to_me(Bullets)[source(M)] : my_health(Health) & Health - Bullets > 0
    <-  ?my_health(Health);
        ...
        -+my_health(NewHealth);
        ?debug(Mode); if (Mode<=1) { .println("HE RECIBIDO ", Bullets, " DISPAROS. Mi salud es :", NewHealth); }
...
+shooting_to_me(Bullets)[source(M)] : my_health(Health) & Health - Bullets <= 0
    <- 	-+my_health(0);
        	-+state(quit);
        	?debug(Mode); if (Mode<=1) { .println("HE RECIBIDO ", Bullets, " DISPAROS. Me muero"); }
        	!!fsm;
        	-shooting_to_me(_)[source(M)].
...
+medic_pack_taken(Medicine)[source(M)]
    <-  ?my_health(Health);
        ...
        ?debug(Mode); if (Mode<=1) { .println("HE RECIBIDO ", Medicine, " SALUD."); }
...
//  SHOT
/////////////////////////////////
+!shot(Shot) : my_ammo(Ammo) & Ammo > 0 & manager(M) & assigned_id(AssignedID) & my_aim_threshold(AimThreshold) & my_shot_threshold(ShotThreshold) //& aimed_agent(_)
